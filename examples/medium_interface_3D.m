...
%% sample properties
N = PPW*[16,32,16]; % size of medium (in pixels)
...
n_sample = [n1*ones(N(1)/2,N(2),N(3));n2*ones(N(1)/2,N(2),N(3)); ];
...
source_amplitude = gausswin(N(3),3) * source_amplitude ;
source_amplitude = permute(source_amplitude, [3,2,1]);
source = Source(source_amplitude, [1,1,1,1]); % horizontally-polarized plane wave source under an angle
...
figure;
imagesc(sim.x_range,sim.y_range,abs(E(:,:,end/2,1)));
