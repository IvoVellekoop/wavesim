...
clear all; close all;
close all; clc; warning off; clear; clear mex; 
addpath('..');
...
slice(x,y,z, log(abs(E)), [0 N(2)/(PPW*4) N(2)/(PPW*2)], [], []);
set(findobj(gca,'Type','Surface'),'EdgeColor','none')
axis square;
xlabel('y / \lambda','FontSize',16);
ylabel('x / \lambda','FontSize',16);
zlabel('z / \lambda','FontSize',16);
h = colorbar;
set(get(h,'Title'),'String','log|E|','FontSize',18,'FontName','Times New Roman');
set(gca,'FontSize',14);
xlim([0 N(2)/(PPW*2)+1]);
ylim([-N(1)/(PPW*2)-1 N(1)/(PPW*2)+1]);
zlim([-N(3)/(PPW*2)-1 N(3)/(PPW*2)+1]);
